...
class EstimatorSS:
    ...
    def _estimate_model(self, observation, t):
        ...
        if time_in_est_period >= self.model_est_period:
            ...
            try:
                ...
                SSest = sippy.system_identification(
                    ...
                    # SS_f=int(self.data_buffer_size/12),
                    # SS_p=int(self.data_buffer_size/10),
                    SS_PK_B_reval=False,
                    tsample=self.sampling_time,
                )

                self.model.update_pars(SSest.A, SSest.B, SSest.C, SSest.D)
            ...
            if self.model_est_checks > 0:
                ...
                for k in range(self.model_est_checks):
                    ...
                    initial_guessest, _, _, _ = np.linalg.lstsq(C, observation)
                    Yest, _ = dss_sim(
                        A, B, C, D, self.action_buffer, initial_guessest, observation
                    )
                ...
                # R  = '\033[31m'
                # Bl  = '\033[30m'
                # initial_guessest,_,_,_ = np.linalg.lstsq(ctrlStat.C, observation)
                # Yest,_ = dssSim(ctrlStat.A, ctrlStat.B, ctrlStat.C, ctrlStat.D, ctrlStat.action_buffer, initial_guessest, observation)
        ...
        initial_guessest, _, _, _ = np.linalg.lstsq(self.model.C, observation)
        self.model.updateIC(initial_guessest)
...
class Estimator_RNN:
    ...
    def __init__(
        ...
        self.observation_buffer = np.zeros(
            (self.data_buffer_size, self.dim_observation)
        )
        self.action_buffer = np.zeros((self.data_buffer_size, self.dim_action))
    ...
    def receive_sys_IO(self, t, observation, action):
        ...
        time_in_sample = t - self.controller_clock

        if time_in_sample >= self.sampling_time:  # New sample
            self.controller_clock = t
    ...
    def output_loss(self):
        ...
        for i in range(self.data_buffer_size - 1):
